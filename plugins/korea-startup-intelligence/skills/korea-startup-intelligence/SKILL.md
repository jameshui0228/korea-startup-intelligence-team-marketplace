...
description: "한국의 약한 시장 신호와 아직 충분히 해결되지 않은 고객 문제를 찾아 블루오션 사업 가설로 발전시키고, 아이디어를 검증·실행·보류·폐기까지 지속 관리할 때 사용한다. 최신 트렌드, 해외→한국 공백, 경쟁·대안, MVP, BM, 고객 실험과 개인 창업 포트폴리오 요청에 적용한다. 공모전·지원사업은 명시적으로 요청한 경우에만 선택 모듈로 다룬다."
---
...
초기 신호 → 고객 행동·현재 지출 → 공급 공백 → 한국 시장/대체재 → 사업 가설 → 검증·실행·보류·폐기로 연결한다.
