...
description: "한국의 약한 시장 신호와 구조 변화를 평범한 재포장이 아닌 참신한 프런티어 가설로 발산·반증하고, 해결되지 않은 고객 문제를 블루오션 사업 가설로 발전시킨다. 개인 창업자의 적합성·시간·예산·WIP·인터뷰/MVP/가격/GTM·출시 KPI·보류/폐기/재개까지 운영할 때 사용한다. 최신 트렌드, 해외→한국 공백, 산업 간 이식, 역트렌드, 경쟁·대안, 고객 실험 요청에 적용한다. 공모전·지원사업은 명시적으로 요청한 경우에만 선택 모듈로 다룬다."
---
...
참신한 아이디어 요청에서는 보수적 검증 게이트를 처음부터 적용해 평범한 후보만 남기지 않는다.
먼저 구조 변화 → 10개 발상 원리 → 8개 사업구조를 조합해 30개를 발산하고, 10개를 반증한 뒤 3개를 선별한다.
탐색 가설(Frontier), 초기 신호(Emerging), 즉시 관측 가능 후보(Executable)를 분리하며 어떤 등급도 성공확률이 아니다.
시의성은 참신성과 별도 벡터로 판단한다. 최근 사건일·원문 검토·독립 생산자·신호 계열을 대조하고,
현재 자료만으로 대중보다 먼저 발견했다거나 미래 유행을 예측했다고 주장하지 않는다.
...
| 요청 | 읽을 참조 | 결과 |
|---|---|---|
| 참신한·혁신적·트렌드 선행 아이디어, 기존 결과가 평범함 | [프런티어 발견](references/frontier-discovery.md), [트렌드 검토](references/trend-radar.md) | 변화 원자→30개 발산→10개 반증→3개 선별 + 등급 분리 |
...
- 보수적 검증과 대담한 발산을 혼동하지 않는다. 근거가 약한 새 가설은 Frontier로 제시할 수 있지만, 평범한 제안을 혁신으로 포장하지 않는다.
